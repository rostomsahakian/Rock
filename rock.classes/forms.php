        ...
        if ($type != NULL) {
            switch ($type) {
                case "update_page_details":
                    ...
                    $clean_page_alias = str_replace(" ", "-", $page_form_data[0]);
        ...
        if ($brand_data_request != NULL) {
            ...
            foreach ($get_all_data_for_brand as $cat_data) {
                ...
                if (count($check_db_for_sub_pages) > 0) {
                    ...
                    $clean_page_alias = str_replace(" ", "-", $cat_data[$brand_data_request['choice']]);
                    $page_alias_with_out_ands = str_replace("&", "and", $clean_page_alias);
                    $page_alias_kosher = preg_replace('/[^a-zA-Z0-9,-]/', "-", $page_alias_with_out_ands);
                    $page_alias = trim(strtolower($page_alias_kosher));
            ...
            if ($brand_data_request['choice'] == "gender") {
                ...
                foreach ($for_categories as $sc) {
                    ...
                    for ($i = 0; $i < count($get_all_data_for_sub_cat); $i++) {
                        ...
                        $clean_page_alias = str_replace(" ", "-", $get_all_data_for_sub_cat[$i]['category']);
                        $page_alias_with_out_ands = str_replace("&", "and", $clean_page_alias);
                        $page_alias_kosher = preg_replace('/[^a-zA-Z0-9,-]/', "-", $page_alias_with_out_ands);
                        $page_alias = trim(strtolower($page_alias_kosher));
                ...
                foreach ($for_categories as $sg) {
                    ...
                    for ($i = 0; $i < count($get_all_data_for_sub_gender); $i++) {
                        ...
                        $clean_page_alias = str_replace(" ", "-", $get_all_data_for_sub_gender[$i]['gender']);
                        $page_alias_with_out_ands = str_replace("&", "and", $clean_page_alias);
                        $page_alias_kosher = preg_replace('/[^a-zA-Z0-9,-]/', "-", $page_alias_with_out_ands);
                        $page_alias = trim(strtolower($page_alias_kosher));
        ...
        if ($data != NULL) {
            ...
            foreach ($get_all_cat_names as $category_name) {
                ...
                if (count($check_pages_table) > 0) {
                    ...
                    $clean_page_alias = str_replace(" ", "-", $category_name['category']);
                    $page_alias_with_out_ands = str_replace("&", "and", $clean_page_alias);
                    $page_alias_kosher = preg_replace('/[^a-zA-Z0-9,-]/', "-", $page_alias_with_out_ands);
                    $page_alias = trim(strtolower($page_alias_kosher));
